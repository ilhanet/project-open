...
const AdminPosts = () => {
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      try {
        const res = await axios.get(`/api/admin/posts/${path}`);
        if (res.data && res.data.status === 'on') {
          setPost(res.data);

          const text1 = res.data.text
  ...
  useEffect(() => {
    if (post.groupId && post.groupId.length > 2) {
      const fetchGroup = async () => {
        try {
          const res = await axios.get(`/api/admin/groups/${post.groupId}`);
          setGroups(res.data);
        } catch (error) {
  ...
  useEffect(() => {
    const fetchUser = async () => {
      try {
        const res = await axios.get(`/api/admin/user/id/${post.userId}`);
        setUser(res.data);  
      } catch (error) {
        console.error("Erro ao buscar usuário:", error);
      }
  ...
  useEffect(() => {
    if (post.repostId) {
      const fetchPosts = async () => {
        try {
          const res = await axios.get(`/api/admin/posts/${post.repostId}`);
          setRepost(res.data);
        } catch (error) {
          console.error("Erro ao buscar posts:", error);
        }
  ...
  useEffect(() => {
      const formattedDate = moment(post.createdAt).tz(timezone).format('DD, MMMM, YYYY HH:mm');
  ...
  useEffect(() => {
    const formattedDate = moment(post.updatedAt).tz(timezone).format('DD, MMMM, YYYY HH:mm');
  ...
  const sanitizedHTML = DOMPurify.sanitize(post.text);
  ...
  return (
    <aside id="Posts" className={post.type === 3 ? ("Container ArticlePost noheaderMenu"):("Container noheaderMenu")}>
      ...
      {user && user.username && post.type === 1 && (<Helmet>
        <title>Post de @{user.username} / Ilhanet</title>
      </Helmet>)}

      {post && sanitizedHTML && post.type === 2 && <Helmet>
        <title>{sanitizedHTML} / Ilhanet</title>
      </Helmet>}

      {post && post.titleArticle && post.type === 3 && <Helmet>
        <title>{post.titleArticle} / Ilhanet</title>
      </Helmet>}
          ...
          <div id="Post">
            ...
            {post.type === 3 && (
              <div className="Article">
                ...
                <div className="textArticle2" dangerouslySetInnerHTML={{ __html: sanitizedHTML}} />
              </div>
            )}
            ...
            {currentUser && isBlock && (
              <form className="NewComment gap-2" onSubmit={handleSubmit(onSubmit)}>
                ...
                <div className="inputComment">
                  <Textarea id="comment" {...register("comment")} placeholder="Digite aqui." onChange={
