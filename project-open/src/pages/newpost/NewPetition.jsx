...
const NewPetition = () => {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      const res = await axios.post(`/api/petitions/create`, { 
        title: data.title, 
        goal: data.goal, 
        text: value, 
        photo: photos,
      ...
      toast(t("Publicado"), {action: {label: t("Ver"), onClick: () => navigate(`/petition/${res.data._id}`)}})
      navigate(-1);
    } catch (error) {
    ...
    <aside id="NewPost" className="Container noheaderMenu">
      ...
      <div className="Main">
        <form className="grid w-full gap-2" onSubmit={handleSubmit(onSubmit)}>
          <h1 className="titleGroup"><b>{t("Criar petição")}</b></h1>
          ...
          <div className="grid gap-3">     
            ...
            <div>
              ...
              <NumericFormat
                ...
                onValueChange={(values) => {
                  setValue("goal", Number(values.value));
                }}
                isNumericString
              />
              <div className="detailsInput">
            ...
            <div>
              ...
              <div className="detailsInput">
                <label className="error-text">
                  {errorContent && <span>{t("errorContent")}</span>}
                </label>
              </div>
            </div>

            <Label>{t("Capa de petição")}</Label>
            ...
            {errorUploadLimit && <div><label className="error-text"><span>{t("msgLimitFile3")}</span></label></div>}
