...
const ReportPetition = () => {
  ...
  useEffect(() => {
    const fetchUser = async () => {
      try {
        const res = await axios.get(`/api/petitions/pub/${path}`);
        if (res.data) {
          setPost(res.data)
          setNotFound(false);
        } else {
  ...
  if (!currentUser) {
    navigate(`/auth/login?redirect=/report/petition/${path}`)
  }
  ...
  return (
    <aside id="Report" className="Container noheaderMenu">
      ...
      <Helmet>
        <title>{t("Denunciar petição")} / Ilhanet</title>
      </Helmet>
